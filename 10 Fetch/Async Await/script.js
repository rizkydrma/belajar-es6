...
btnSearch.addEventListener('click', async function(){
  ...
  const movies = await getMovies(searchMovie.value)
  updateUI(movies)
})
...
function getMovies(keyword){
  return fetch('http://www.omdbapi.com/?apikey=c58f5023&s=' + keyword)
...
function getDetailMovie(id){
  return fetch('http://www.omdbapi.com/?apikey=c58f5023&i=' + id)
    .then(response => response.json())
    .then(response => response)
}

function updateModalUI(movieDetail){
  const modalDetail = showDetailMovie(movieDetail)
